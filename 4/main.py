# -*- coding: utf-8 -*-
# @Time    : 2023/2/14 16:40
# @Author  : Blue
# @File    : main.py

'''
    图片数据对应成数字 然后根据数字图片宽度算出偏移量
'''
...
def css_sort(arr):
    ...
    for i in range(n):
        # 用一个标志变量记录是否发生过交换
        # 如果在一次遍历中没有发生交换，说明序列已经有序，可以提前结束循环
        swapped = False
        for j in range(n - 1 - i):
            if float(arr[j][-1]) == 0 or float(arr[j + 1][-1]) == 0:
                continue

            if float(arr[j][-1]) > float(arr[j + 1][-1]):
...
def main():
    ...
    for page in range(1, 6):
        ...
        response = s.get('https://match.yuanrenxue.com/api/match/4', params=params)
        result = response.json()
        key = result['key']
        value = result['value']
        j_key = hashlib.md5(base64.b64encode((key + value).encode('utf-8')).rstrip(b"=")).hexdigest()
        info = result['info']

        doc = pq(info)
        for td in doc.find('td').items():
            td_list = []
            offset = 1
            for img in td.find('img').items():
                if j_key in img.attr('class'):
                    continue
                src = img.attr('src')
                style = img.attr('style')
                num = img_to_num[src]
                px = style.lstrip('left:').rstrip('px')
                # 每个数字占11px 这里算出偏移量
                td_list.append((num, float(px) + 11 * offset))
                offset += 1
            print(''.join([str(i[0]) for i in sorted(td_list, key=lambda x: x[-1])]))
            sum_total += int(''.join([str(i[0]) for i in sorted(td_list, key=lambda x: x[-1])]))
